...
class Optimizer_Adam:
    ...
    def update_params(self, layer):
        ...
        weight_momentums_correceted = layer.weight_momentums / (1-self.beta_1 ** (self.iterations +1))
        bias_momentums_correceted = layer.bias_momentums / (1-self.beta_1 ** (self.iterations +1))

        layer.weight_cache = self.beta_2 * layer.weight_cache + (1-self.beta_2) * layer.dweights**2
        layer.bias_cache = self.beta_2 *layer.bias_cache + (1-self.beta_2) * layer.dbiases ** 2

        weight_cache_correceted = layer.weight_cache / (1-self.beta_2 ** (self.iterations +1))
        bias_cache_correceted = layer.bias_cache / (1-self.beta_2 ** (self.iterations +1))

        layer.weights += -self.current_rate * weight_momentums_correceted / (np.sqrt(weight_cache_correceted) + self.epsilon)
        layer.biases += -self.current_rate * bias_momentums_correceted / (np.sqrt(bias_cache_correceted) + self.epsilon)
...
#optimizer = Optimizer_SGD(0.1, 5e-7, 0.1)
#optimizer = Optimizer_Adagrad(0.5, 5e-7)
#optimizer = Optimizer_RMSprop(0.1,1e-7, 0.1)
#optimizer = Optimizer_Adam(0.3, 5e-7, 1)
